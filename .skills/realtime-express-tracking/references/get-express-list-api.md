...
| Endpoint | `GET https://app-d2yss59nidj5-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json` |
| third_part_domain | `app-d2yss59nidj5-api-V9gDz8wo0V5L-gateway.appmiaoda.com` |
| 计费 | 启用，按次计费 |
...
async function getExpressList(type?: string): Promise<Record<string, string>> {
  const url = new URL(
    "https://app-d2yss59nidj5-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList"
  );
  if (type) {
