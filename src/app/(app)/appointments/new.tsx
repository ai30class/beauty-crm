...
export default function NewAppointmentScreen() {
  ...
  const selectFoundCustomer = (result: { id: string; name: string }) => {
    ...
    setSelectedCustomer({
      ...
      created_at: '', updated_at: '', booking_restricted: false, booking_allowed_hours: [], no_show_count: 0, tags: [], referral_source: null,
    });
  ...
  const handleCreateNewCustomer = async () => {
    ...
    try {
      ...
      const created = await createCustomerAndGetId({
        ...
        booking_restricted: false, booking_allowed_hours: [], no_show_count: 0, tags: [], referral_source: null,
      });
  ...
  const handleSave = async () => {
    ...
    try {
      await createAppointment({
        ...
        // 存下選的是哪個服務項目（migration 00091）：預約詳情頁靠這個判斷要不要提醒簽同意書
