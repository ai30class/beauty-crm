  ...
  return (
    <KeyboardAvoidingView behavior={process.env.EXPO_OS === 'ios' ? 'padding' : 'height'} className="flex-1">
      ...
      <View className="flex-row items-center px-5 pt-14 pb-4 bg-background">
        ...
        <Text className="font-rounded text-xl font-bold text-foreground flex-1">編輯顧客</Text>
      </View>
      ...
      >
        <View>
          <Text className="font-rounded text-sm font-medium text-foreground mb-1.5">姓名 *</Text>
          <TextInput className="bg-card border border-border rounded-2xl px-4 font-rounded text-base text-foreground" style={{ height: 52 }} placeholder="顧客姓名" placeholderTextColor="#c4a0ae" value={name} onChangeText={setName} />
        </View>
        <View>
          <Text className="font-rounded text-sm font-medium text-foreground mb-1.5">電話 *</Text>
          <TextInput className="bg-card border border-border rounded-2xl px-4 font-rounded text-base text-foreground" style={{ height: 52 }} placeholder="手機號碼" placeholderTextColor="#c4a0ae" value={phone} onChangeText={setPhone} keyboardType="phone-pad" />
        </View>
        <View>
          <Text className="font-rounded text-sm font-medium text-foreground mb-1.5">生日</Text>
          <Pressable className="bg-card border border-border rounded-2xl px-4 items-start justify-center active:opacity-80" style={{ height: 52 }} onPress={() => setShowDatePicker(!showDatePicker)}>
        ...
        <Pressable className="bg-primary rounded-2xl items-center justify-center active:opacity-80 mt-2" style={{ height: 56 }} onPress={handleSave} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text className="font-rounded text-white text-base font-semibold">儲存變更</Text>}
        </Pressable>
      </ScrollView>
