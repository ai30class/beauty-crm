...
import {
  ...
  getShopPaymentSettings, upsertShopPaymentSettings, getStaffForPicker,
} from '@/db/api';
...
export default function ShopSettingsScreen() {
  ...
  // 設計師自己設定的個人封鎖時段（staff_id 有值）也會出現在這份清單，標上是哪位設計師
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setStaffNames(Object.fromEntries(roster.map(r => [r.id, r.name])));
      } finally {
        setLoading(false);
      }
    })();
          ...
          {blockedSlots.length === 0 ? (
            ...
            blockedSlots.map((s, i) => (
              <View key={s.id} className={`flex-row items-center py-2.5 ${i > 0 ? 'border-t border-border' : ''}`}>
                <View className="flex-1">
                  <Text className="font-rounded text-sm font-semibold text-foreground">{s.staff_id ? `${staffNames[s.staff_id] ?? '設計師'} 的個人封鎖` : (s.label || '封閉時段')}</Text>
